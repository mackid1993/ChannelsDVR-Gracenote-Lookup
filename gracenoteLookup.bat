...
ECHO # Add quoted exact phrase searches >> "%_PSSCRIPT%"
ECHO $searchTerms += @{term = $channel; quoted = $true} >> "%_PSSCRIPT%"
ECHO $searchTerms += @{term = $channel.ToLower(); quoted = $true} >> "%_PSSCRIPT%"
ECHO # Add unquoted OR searches >> "%_PSSCRIPT%"
ECHO $searchTerms += @{term = $channel; quoted = $false} >> "%_PSSCRIPT%"
ECHO $searchTerms += @{term = $channel.ToLower(); quoted = $false} >> "%_PSSCRIPT%"
ECHO $searchTerms += @{term = $stripped; quoted = $false} >> "%_PSSCRIPT%"
ECHO $searchTerms += @{term = $stripped.ToLower(); quoted = $false} >> "%_PSSCRIPT%"
ECHO if ($stripped.Length -le 4) { >> "%_PSSCRIPT%"
ECHO     $baseName = $stripped.ToLower() >> "%_PSSCRIPT%"
ECHO     $searchTerms += @{term = $baseName + 'tv'; quoted = $false} >> "%_PSSCRIPT%"
ECHO     $searchTerms += @{term = $baseName + 'network'; quoted = $false} >> "%_PSSCRIPT%"
ECHO     $searchTerms += @{term = $baseName + 'hd'; quoted = $false} >> "%_PSSCRIPT%"
ECHO     $searchTerms += @{term = $baseName + 'channel'; quoted = $false} >> "%_PSSCRIPT%"
ECHO } >> "%_PSSCRIPT%"
ECHO $allResults = @() >> "%_PSSCRIPT%"
ECHO $foundCount = 0 >> "%_PSSCRIPT%"
ECHO foreach ($search in $searchTerms) { >> "%_PSSCRIPT%"
ECHO     $searchTerm = $search.term >> "%_PSSCRIPT%"
ECHO     if ($search.quoted) { >> "%_PSSCRIPT%"
ECHO         $encoded = [System.Web.HttpUtility]::UrlEncode('"' + $searchTerm + '"') >> "%_PSSCRIPT%"
ECHO         Write-Host "Trying quoted: `"$searchTerm`"" >> "%_PSSCRIPT%"
ECHO     } else { >> "%_PSSCRIPT%"
ECHO         $encoded = [System.Web.HttpUtility]::UrlEncode($searchTerm) >> "%_PSSCRIPT%"
...
ECHO             if ($json -and $json.Count -gt 0) { >> "%_PSSCRIPT%"
ECHO                 $allResults += $json >> "%_PSSCRIPT%"
ECHO                 $foundCount++ >> "%_PSSCRIPT%"
ECHO                 Write-Host "  Found $($json.Count) result(s)" -ForegroundColor Green >> "%_PSSCRIPT%"
ECHO             } >> "%_PSSCRIPT%"
ECHO         } >> "%_PSSCRIPT%"
...
ECHO     Write-Host "Search Summary: $foundCount searches returned results" >> "%_PSSCRIPT%"
ECHO     Write-Host "Total results collected: $($allResults.Count)" >> "%_PSSCRIPT%"
ECHO     $uniqueResults = $allResults ^| Group-Object -Property stationId ^| ForEach-Object { $_.Group[0] } >> "%_PSSCRIPT%"
ECHO     Write-Host "Unique stations found: $($uniqueResults.Count)" >> "%_PSSCRIPT%"
ECHO     Write-Host "========================================" >> "%_PSSCRIPT%"
ECHO     Write-Host "" >> "%_PSSCRIPT%"
ECHO     $uniqueResults ^| Sort-Object type, name ^| ForEach-Object { >> "%_PSSCRIPT%"
ECHO         $logo = if ($_.preferredImage.uri) { " [Logo available]" } else { "" } >> "%_PSSCRIPT%"
ECHO         Write-Host "  [$($_.type)] $($_.name) - $($_.callSign) - StationID: $($_.stationId)$logo" >> "%_PSSCRIPT%"
...
ECHO     } >> "%_PSSCRIPT%"
ECHO } else { >> "%_PSSCRIPT%"
ECHO     Write-Host 'No results found from any search variation' -ForegroundColor Yellow >> "%_PSSCRIPT%"
ECHO     Write-Host '========================================' >> "%_PSSCRIPT%"
ECHO } >> "%_PSSCRIPT%"
GOTO :EOF
